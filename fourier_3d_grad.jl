...
#x_perm = 20f0 * ones(Float32,64,64)
x_perm = decode(x_normalizer, zeros(Float32,64,64))[:,:,1]
...
for iter = 1:grad_iterations
    ...
    @time grads = gradient(p) do
        out = decode(y_normalizer,NN(perm_to_tensor(encode(x_normalizer,reshape(x_perm,64,64))[:,:,1],nt,grid,dt)))
...
#savefig("result/inversion_weird.png")

x_start = 20 * ones(Float32,nx,ny)
temp = decode(y_normalizer,NN(perm_to_tensor(encode(x_normalizer,reshape(x_start,64,64))[:,:,1],nt,grid,dt)))
figure(figsize=(12,6));
subplot(1,3,1)
imshow(temp[:,:,11,1]);
subplot(1,3,2)
...
subplot(1,3,3)
imshow(temp[:,:,51,1]);
suptitle("CO2 concentration generated by homogeneous 20 mD permeability")
#savefig("result/CO2from20.png")
