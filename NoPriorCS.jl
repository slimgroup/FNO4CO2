...
for j=1:grad_iterations
    ...
    Grad_Loss[j] = loss
    imshow(decode(x),vmin=20,vmax=120)
end
...
subplot(1,3,2);
imshow(x_inv,vmin=20,vmax=120);title("inversion by NN w/o prior, $grad_iterations iter");
subplot(1,3,3);
imshow(x_true,vmin=20,vmax=120);title("GT permeability");
savefig("result/nopriorCS.png", bbox_inches="tight", dpi=300)
