...
x = reshape(encode(x_normalizer, 20f0 * ones(Float32, nx, ny, 1)), nx, ny, 1, 1)
x_init = decode(x_normalizer, reshape(x, nx, ny, 1))[:,:,1]

using Zygote: @adjoint
struct InvertNet
    G::NetworkMultiScaleHINT
end
(G::InvertNet)(x::AbstractArray) = G.G(x)[1]
@adjoint function(G::InvertNet)(x::AbstractArray)
    z = G.G(x)[1]
...
for j=1:grad_iterations
    ...
    @time grads = gradient(θ) do
        ...
        #prior = 0.5f0 * λ^2f0 * norm(G1(x))^2f0
        prior = 0f0
...
subplot(1,3,2);
imshow(x_inv,vmin=20,vmax=120);title("inversion by NN w/o NF prior, $grad_iterations iter");
subplot(1,3,3);
imshow(x_test_1,vmin=20,vmax=120);title("GT permeability");
savefig("result/noNFpriorfromsat.png", bbox_inches="tight", dpi=300)
