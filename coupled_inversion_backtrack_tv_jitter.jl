...
try
    JLD2.@load "data/data/time_lapse_data_$(nv)nv_$(nsrc)nsrc_jitter.jld2" d_obs
    global d_obs = d_obs
...
for j=1:grad_iterations
    ...
    fig.savefig("result/jittertvperm$(j).png", bbox_inches="tight",dpi=300)
    figloss.savefig("result/jittertvloss$(j).png", bbox_inches="tight",dpi=300)
    figmisfit.savefig("result/jittertvmisfit$(j).png", bbox_inches="tight",dpi=300)
    figprior.savefig("result/jittertvprior$(j).png", bbox_inches="tight",dpi=300)

    println("Coupled inversion iteration no: ",j,"; function value: ",fval)
    JLD2.@save "result/jittertvcoupleinversioniter$(j).jld2" x fval gvec hisloss hismisfit hisprior
end
