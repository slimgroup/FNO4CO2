...
JLD2.@save "data/data/timelapse_$(nsrc)nsrc_$(nv)nv_crossreflect.jld2" d_obs
...
suptitle("$nv vintages, $nsrc each, crosswell+reflect")

figure();
plot(Grad_Loss)

JLD2.@save "result/coupleinv$(grad_iterations)_$(nv)nv_$(nsrc)nsrc_crossreflect.jld2" x_inv Grad_Loss
