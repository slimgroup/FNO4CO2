...
function SpectralConv(in_channels::Int64, out_channels::Int64, modes::Vector{Int64}; DT::DataType=ComplexF32)
    scale = 1f0 / convert(real(DT), sqrt(in_channels * out_channels))
    Random.seed!(1234)
    # weights = scale*rand(DT, modes..., in_channels, out_channels, 2^(length(modes)-1))
    weights = rand(DT, 1, 1, 1, in_channels, out_channels, 1) ./ convert(real(DT), sqrt(in_channels * out_channels))
...
function compl_mul(x::AbstractArray{T, 5}, y::Matrix{T}) where T
    # complex multiplication
    # x in (modes1, modes2, modes3, input channels, batchsize)
    # y in (input channels, output channels)
    # output in (modes1,modes2,modes3,output channels,batchsize)
    x_per = permutedims(x,[5,4,1,2,3]) # batchsize*in_channels*modes1*modes2*modes3
    y_per = permutedims(y,[1,2]) # in_channels*out_channels
    x_resh = reshape(x_per,size(x_per,1),size(x_per,2),:) # batchsize*in_channels*(modes1*modes2*modes3)
    y_resh = y_per # reshape(y_per,size(y_per,1),size(y_per,2),:) # in_channels*out_channels
    out_resh = batched_mul(x_resh,y_resh) # batchsize*out_channels*(modes1*modes2*modes3)
    out_per = reshape(out_resh,size(out_resh,1),size(out_resh,2),size(x,1),size(x,2),size(x,3)) # batchsize*out_channels*modes1*modes2*modes3
    out = permutedims(out_per,[3,4,5,2,1])
    return out
...
function (L::SpectralConv)(x::AbstractArray{T, 5}) where T
    ...
    (modes1, modes2, modes3) = (4, 4, 4) # size(L.weights)[[1,2,3]]

    ## only keep low frequency coefficients L.weights[1,1,1,:,:,1]
    out_ft = cat(cat(cat(compl_mul(x_ft[1:modes1, 1:modes2, 1:modes3, :,:], L.weights[1,1,1,:,:,1]),
                zeros(Complex{T}, modes1, modes2, size(x_ft,3)-2*modes3, size(x_ft,4), size(x_ft,5)), 
                compl_mul(x_ft[1:modes1, 1:modes2, end-modes3+1:end,:,:], L.weights[1,1,1,:,:,1]),dims=3),
                zeros(Complex{T}, modes1, size(x_ft, 2)-2*modes2, size(x_ft,3), size(x_ft,4), size(x_ft,5)),
                cat(compl_mul(x_ft[1:modes1, end-modes2+1:end, 1:modes3,:,:], L.weights[1,1,1,:,:,1]),
                zeros(Complex{T}, modes1, modes2, size(x_ft,3)-2*modes3, size(x_ft,4), size(x_ft,5)),
                compl_mul(x_ft[1:modes1, end-modes2+1:end, end-modes3+1:end,:,:], L.weights[1,1,1,:,:,1]),dims=3)
                ,dims=2),
                zeros(Complex{T}, size(x_ft,1)-modes1, size(x_ft,2), size(x_ft,3), size(x_ft,4), size(x_ft,5)),dims=1)

    # out_ft = cat(cat(cat(x_ft[1:modes1, 1:modes2, 1:modes3, :,:],
    # zeros(Complex{T}, modes1, modes2, size(x_ft,3)-2*modes3, size(x_ft,4), size(x_ft,5)), 
    # x_ft[1:modes1, 1:modes2, end-modes3+1:end,:,:],dims=3),
    # zeros(Complex{T}, modes1, size(x_ft, 2)-2*modes2, size(x_ft,3), size(x_ft,4), size(x_ft,5)),
    # cat(x_ft[1:modes1, end-modes2+1:end, 1:modes3,:,:],
    # zeros(Complex{T}, modes1, modes2, size(x_ft,3)-2*modes3, size(x_ft,4), size(x_ft,5)),
    # x_ft[1:modes1, end-modes2+1:end, end-modes3+1:end,:,:],dims=3)
    # ,dims=2),
    # zeros(Complex{T}, size(x_ft,1)-modes1, size(x_ft,2), size(x_ft,3), size(x_ft,4), size(x_ft,5)),dims=1)
...
function Net3d(modes::Vector{Int64}, width::Int64; in_channels::Int64=4, out_channels::Int64=1, mid_channels::Int64=128)
    ...
    function gen(shape...)
        ...
        return Flux.glorot_uniform(rng, shape...)
        # return rand(Float32, shape...) / convert(Float32, sqrt(prod(shape)))
    end

    return Net3d(
        Conv((1, 1, 1), in_channels=>width; init=gen, bias=false),
        SpectralConv(width, width, modes),
        SpectralConv(width, width, modes),
        SpectralConv(width, width, modes),
        SpectralConv(width, width, modes),
        Conv((1, 1, 1), width=>width; init=gen, bias=false),
        Conv((1, 1, 1), width=>width; init=gen, bias=false),
        Conv((1, 1, 1), width=>width; init=gen, bias=false),
        Conv((1, 1, 1), width=>width; init=gen, bias=false),
        BatchNorm(width, identity; affine=false, track_stats=false, ϵ=1f-5, momentum=1f-1),
        BatchNorm(width, identity; affine=false, track_stats=false, ϵ=1f-5, momentum=1f-1),
        BatchNorm(width, identity; affine=false, track_stats=false, ϵ=1f-5, momentum=1f-1),
        BatchNorm(width, identity; affine=false, track_stats=false, ϵ=1f-5, momentum=1f-1),
        Conv((1, 1, 1), width=>mid_channels; init=gen, bias=false),
        Conv((1, 1, 1), mid_channels=>out_channels; init=gen, bias=false)
    )
    return block
end
