...
nc_in = 1 + 2 + 4 # CIG + 2 velocity models + indices
nc_out = 1

@info "Initializing model..."

modelConfig = DFNO_3D.ModelConfig(nc_in=nc_in, nc_lift=width, nc_out=nc_out, nx=n[1], ny=n[2], nz=offsets, nt=1, partition=partition, dtype=Float32)

dataset_path = "/pscratch/sd/r/richardr/FNO-CIG/results/concatenated_data.jld2"
x_train, y_train, x_valid, y_valid = read_velocity_cigs_offsets_as_nz(dataset_path, modelConfig, ntrain=ntrain, nvalid=nvalid)

x_train = reshape(x_train, nc_in, n..., offsets, :)
x_valid = reshape(x_valid, nc_in, n..., offsets, :)
y_valid = reshape(y_valid, nc_out, n..., offsets, :)
y_train = reshape(y_train, nc_out, n..., offsets, :)

nc_in = 1 + 2 + 3 # remove t indices
...
x_train = permutedims(x_train, [2, 3, 4, 1, 5])
x_valid = permutedims(x_valid, [2, 3, 4, 1, 5])
y_valid = permutedims(y_valid, [2, 3, 4, 1, 5])
y_train = permutedims(y_train, [2, 3, 4, 1, 5])

@info "Loaded data..."

println(size(x_train), ":", size(y_train), ":", size(x_valid), ":", size(y_valid))

NN = Net3d(modes, width; in_channels=nc_in, out_channels=nc_out, mid_channels=128)
...
for ep = 1:epochs
    ...
    x_plot_temp = permutedims(x_plot, [4, 1, 2, 3, 5])
    y_plot_temp = permutedims(y_plot, [4, 1, 2, 3, 5])
