...
for j=1:grad_iterations
    ...
    fig.savefig("result/tvperm$(j).png", bbox_inches="tight",dpi=300)
    figloss.savefig("result/tvloss$(j).png", bbox_inches="tight",dpi=300)
    figmisfit.savefig("result/tvmisfit$(j).png", bbox_inches="tight",dpi=300)
    figprior.savefig("result/tvprior$(j).png", bbox_inches="tight",dpi=300)

    println("Coupled inversion iteration no: ",j,"; function value: ",fval)
    JLD2.@save "result/tvcoupleinversioniter$(j).jld2" x fval gvec hisloss hismisfit hisprior
end
